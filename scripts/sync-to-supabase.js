...
// Transform function for evictions_data
// Same pattern as distress/working_poor: zip_code is text, all other columns are numeric
// Note: 'id' is stripped because the Supabase table auto-generates it (PK is 'id' not 'id_no')
const EVICTIONS_TEXT_COLUMNS = ['zip_code'];
function transformEvictionsData(row) {
  const result = {};
  for (const [key, value] of Object.entries(row)) {
    if (key === 'id') continue; // Skip - auto-generated by Supabase
    if (EVICTIONS_TEXT_COLUMNS.includes(key)) {
      result[key] = value || null;
    } else {
      result[key] = parseNumericOrNull(value);
    }
...
// Transform function for zip_code_data
// Same pattern: zip_code and county are text, all other columns are numeric
const ZIP_CODE_DATA_TEXT_COLUMNS = ['zip_code', 'county'];
function transformZipCodeData(row) {
  const result = {};
  for (const [key, value] of Object.entries(row)) {
    if (key === 'id') continue; // Skip - auto-generated by Supabase
    if (ZIP_CODE_DATA_TEXT_COLUMNS.includes(key)) {
      result[key] = value || null;
    } else {
      result[key] = parseNumericOrNull(value);
    }
